...
export function drawGaddiMark(
  ...
  // TOP/BOTTOM -> use nomW (width); LEFT/RIGHT/SHUTTER -> use nomH (height)
  const isLeftRight = /LEFT|RIGHT/i.test(type);
  const isTopBottom = /TOP|BOTTOM/i.test(type);
  const isShutter = /SHUTTER/i.test(type);

  // Skip if panel type is not recognized
  if (!isLeftRight && !isTopBottom && !isShutter) {
    // Unknown panel type -> no gaddi line
    return;
  }

  // SHUTTER panels mark HEIGHT (same as LEFT/RIGHT)
  const markValue = (isLeftRight || isShutter) ? nomH : nomW;
  ...
  // SHUTTER marks HEIGHT (like LEFT/RIGHT)
  if (isTopBottom) {
    // when marking width, it should not be longer than panel.w
    length = Math.min(length, panel.w - 2 * inset);
  } else {
    // when marking height (LEFT/RIGHT/SHUTTER), not longer than panel.h
  ...
  if (isTopBottom) {
    ...
    if (widthOnXAxis) {
      // Width on X-axis: draw horizontal line at top edge
      const yLocal = inset;
      const startX = (panel.w - length) / 2;
      sx = startX; sy = yLocal;
      ex = startX + length; ey = yLocal;
    } else {
      // Width on Y-axis: draw vertical line at left edge
      const xLocal = inset;
    ...
    // LEFT, RIGHT, or SHUTTER: marks HEIGHT
  ...
  try {
    ...
    if (isHorizontal) {
      ...
      doc.text("GADDI", leftX, midY + 7, { align: "left" });
    } else {
      // Vertical line (Y-axis): text at BOTTOM, INSIDE panel with 15px gap (scaled to ~9pt)
...
/**
 * Calculate gaddi line direction based on panel type and dimensions
 * Unifies logic between PDF and Preview
 */
export function calculateGaddiLineDirection(
  ...
  // Logic matches drawGaddiMark:
  if (isTopBottom) {
    // TOP/BOTTOM marks WIDTH
    // If w >= h, Width is X-axis (Horizontal)
    // If h > w, Width is Y-axis (Vertical)
    return w >= h ? 'horizontal' : 'vertical';
  } else {
    // LEFT/RIGHT/SHUTTER marks HEIGHT
    // If w >= h, assume Rotated so Height is X-axis (Horizontal)?? 
    // Wait, reusing exact drawGaddiMark logic:
    // "If panel.w >= panel.h: height is on X-axis"
    return w >= h ? 'horizontal' : 'vertical';
  }
}
