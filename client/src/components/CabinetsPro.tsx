...
import { Cabinet, cabinetSchema, Panel, cabinetTypes, CabinetType } from '@shared/schema';
...
import { buildCuttingListSummary } from '@/lib/summary';
...
import { getMasterDefaults, applyMasterDefaultsToCabinet } from '@/lib/settings/MasterSettingsEngine';
import { getFormDefaultValues } from '@/lib/form/FormDefaultsEngine';
import { prepareCabinet, computeGrainDirections } from '@/lib/submit';
...
// PDF-style cutting sheet preview with all features
...
<svg xmlns="http://www.w3.org/2000/svg" width="${svgWidth}" height="${svgHeight + 120}" viewBox="0 0 ${svgWidth} ${svgHeight + 120}">
  ...
  if (sheets.length === 0) {
    return (
      <div className="text-center py-8 text-slate-400 bg-slate-50 rounded-lg border border-dashed border-slate-300">
        No sheets generated for this material group
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Sheet Navigation Thumbnails */}
      {sheetCount > 1 && (
        <div className="flex items-center gap-2 p-2 bg-slate-100 rounded-lg overflow-x-auto">
          <span className="text-xs font-medium text-slate-600 shrink-0">Sheets:</span>
          {sheets.map((_: any, idx: number) => {
            const placed = sheets[idx].placed || [];
            const eff = calculateEfficiency(placed);
            return (
              <button
                key={idx}
                onClick={() => setActiveSheetIdx(idx)}
                className={`shrink-0 w-12 h-16 rounded border-2 flex flex-col items-center justify-center text-xs transition-all ${
                  idx === activeSheetIdx
              ...
              >
                ...
                <span className={`text-[9px] px-1 rounded ${getEfficiencyColor(eff)} text-white`}>
                  {eff}%
                </span>
              </button>
            );
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 text-xs">
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded ${colorMode === 'type' ? 'bg-blue-500 text-white' : 'bg-slate-200'}`}
            >
              Type
            </button>
            <button
              onClick={() => setColorMode('room')}
              className={`px-2 py-0.5 rounded ${colorMode === 'room' ? 'bg-blue-500 text-white' : 'bg-slate-200'}`}
            >
              Room
            </button>
          </div>
          ...
          <label className="flex items-center gap-1 text-xs cursor-pointer">
            <input
              ...
              className="w-3 h-3"
            />
            <span className="text-slate-500">Show Waste</span>
          </label>
        </div>
        {/* Jump to Sheet */}
        {sheetCount > 3 && (
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="space-y-1">
                      ...
                      >
                        <SelectTrigger className="h-8 text-sm bg-white" tabIndex={1}>
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <SelectTrigger className="h-8 text-sm bg-white" tabIndex={2}>
                          <SelectValue placeholder="Select front laminate..." />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <SelectTrigger className="h-8 text-sm bg-white" tabIndex={3}>
