...
// Check if selected is a center post
const isDraggableCenterPost = (id: string | null): boolean => {
  return id?.startsWith("MOD-POST-") ?? false;
};

// Get center post index
const getCenterPostIndex = (id: string): number => {
  const match = id.match(/MOD-POST-(\d+)/);
  return match && match[1] ? parseInt(match[1]) + 1 : 1;
};

export default function CenterPostOverlay() {
  ...
  const isCenterPost = isDraggableCenterPost(selectedId);
  const isWardrobeCarcass = moduleConfig?.unitType === "wardrobe_carcass";

  // Only show for center posts in wardrobe carcass
  if (!selectedId || !isCenterPost || !isWardrobeCarcass) {
    return null;
  }

  const postIndex = getCenterPostIndex(selectedId);
  ...
  return (
    <div style={getOverlayContainerStyle("centerPost")}>
      ...
      {/* Move/Resize toggle - mutually exclusive */}
