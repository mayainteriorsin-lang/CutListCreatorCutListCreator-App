...
export const QuotationItemRow = memo(forwardRef<HTMLTableRowElement, QuotationItemRowProps>(function QuotationItemRow({
  ...
  return (
    <tr ref={ref} className={cn(
      ...
      {/* Row number */}
      <td className="py-0.5 sm:py-1 px-0.5 sm:px-1 text-center">
        <span className="w-4 h-4 sm:w-5 sm:h-5 bg-slate-100 rounded-full flex items-center justify-center text-[9px] sm:text-[10px] font-medium text-slate-500">
          {itemNumber}
        </span>
      </td>

      {/* Description */}
      <td className="py-0.5 sm:py-1 px-0.5 sm:px-1">
        <Input
          value={row.name || ''}
          onChange={(e) => handleFieldChange('name', e.target.value)}
      ...
      <td className="py-0.5 sm:py-1 px-0.5 sm:px-1">
        <Input
          ...
          className="h-6 sm:h-7 text-[10px] sm:text-xs text-center w-full"
          placeholder="H"
        />
      </td>
      ...
      <td className="py-0.5 sm:py-1 px-0.5 sm:px-1">
        <Input
          ...
          className="h-6 sm:h-7 text-[10px] sm:text-xs text-center w-full bg-slate-50"
          placeholder="-"
          readOnly
        />
      </td>
      ...
      {/* Total (calculated) */}
      <td className="py-0.5 sm:py-1 px-1 sm:px-2 text-right text-[10px] sm:text-xs font-semibold text-slate-700">
        {row.total ? `₹${formatCurrency(Number(row.total))}` : '-'}
      </td>

      {/* Actions - always visible on mobile (touch doesn't have hover) */}
      <td className="py-0.5 sm:py-1 px-0.5 sm:px-1">
