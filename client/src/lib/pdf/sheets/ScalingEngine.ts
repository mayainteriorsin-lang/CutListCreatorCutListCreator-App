...
export function computeSheetScaling(params: ScalingParams): ScalingResult {
  const { sheetW, sheetH, pageW, margin, diagramStartY, diagramEndY } = params;
  ...
  // Center the sheet in the available diagram space
  const offsetX = margin + (diagramW - scaledW) / 2;
